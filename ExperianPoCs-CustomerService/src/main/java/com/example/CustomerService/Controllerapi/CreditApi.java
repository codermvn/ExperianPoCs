...
public interface CreditApi {
    ...
    Logger log = LoggerFactory.getLogger(CreditApi.class);
    ...
    @ApiOperation(value = "credit amount", nickname = "creditPost", notes = "credit amount ", response = Customer.class, tags={ "customers", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "amount credited", response = Customer.class),
        @ApiResponse(code = 500, message = "internal server error", response = ExceptionResponse.class),
        @ApiResponse(code = 404, message = "Customer could not be found", response = ExceptionResponse.class),
        @ApiResponse(code = 409, message = "incase someone sends negative amount", response = ExceptionResponse.class) })
    @RequestMapping(value = "/credit",
        produces = { "application/json", "application/xml" }, 
        consumes = { "application/json", "application/xml" },
        method = RequestMethod.POST)
    default ResponseEntity<Customer> creditPost(@ApiParam(value = "amount to be credited"  )  @Valid @RequestBody CustomerAmount body
) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            ...
            log.warn("ObjectMapper or HttpServletRequest not configured in default CreditApi interface so no example is generated");
        }
